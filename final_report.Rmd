...
cpad_nature_count <- cpad %>%
  mutate(pt_count = lengths(st_intersects(cpad, nature_sf))) %>%
  filter(pt_count > 0)
...
cpad_all_count <- cpad %>%
  mutate(nature_count = lengths(st_intersects(cpad, nature_sf)),
...
```{r}
cpad_locals <- cpad %>%
  mutate(nature_count = lengths(st_intersects(cpad, nature_sf %>% filter(user_type == "local"))),
...
cpad_tourists <- cpad %>%
  mutate(nature_count = lengths(st_intersects(cpad, nature_sf %>% filter(user_type == "tourist"))),
