...
leaflet(tweet_data) %>%
  # Base groups
  addProviderTiles(providers$CartoDB.Positron, options = providerTileOptions(minZoom = 10)) %>%
  # Overlay groups %>%
...
#display <- dictionary
kable(dictionary[order(dictionary$word),]) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), font_size = 11, fixed_thead = T)
```
...
Was there a time when twitter changed how geotag? does that explain any decrease in geotagging tweets?

The ^ carrot might force string to start with the word rather than find just that word.

Hex assessemnt for % nature tweets

Julien - TFIDF for twitter words. https://en.wikipedia.org/wiki/Tf–idf

Could look at seasons. Do we see terms associated with fire/drought/rain? 

Look at retweets/favorites for nature-based? Could use them as weighting factor.
