...
ggplot(scatter_df, aes(x = local, y = tourist, label = label)) +
  ...
  labs(x = "Locals", y = "Tourists", title = "Proportion of nature-based tweets by locals & tourists at Santa Barbara protected areas.",
  subtitle = "Size indicates total number of tweets") +
...
We have `r length(unique(tweets_in_cpad$user_id))` unique users from within CPAD areas, representing `r round(length(unique(tweets_in_cpad$user_id))/length(unique(tweet_data$user_id))*100,0)`% of all unique users in the dataset.
