...
- Nature-based tweets were identified using text detection by comparing tweet content to a dictionary of nature-based words
...
p1 <- ggplot(twt_df, aes(x = local, y = tourist)) +
  ...
  labs(x = "Locals", y = "Tourists", title = "Proportion of users visiting Santa Barbara \n protected areas",
       subtitle = "Points in shaded box displayed to the right") +
...
labels <- data.frame(type = c("All", "Nature based", "Non-nature based"), 
...
p2 <- ggplot(data = allsb, aes(x = date, y = avg_score, color = type)) +
  ...
  scale_x_date(expand = expand_scale(mult = c(0, 0.1))) +
  scale_colour_manual(values = c("black", "black", "darkgreen","gray30", "darkgreen", "gray50"),
                      labels = c("All", "Nature-based", "Non nature-based"))
...
- tourists vs locals. If we had access to a larger twitter dataset, we could improve how we identify tourists and locals by better seeing where tourists are "from" (or where they tweet more consistently)
- better CPAD dataset
