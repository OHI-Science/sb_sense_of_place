...
The number of geotagged tweets is going down over time. There is a significant drop in tweets around April 30, 2015.
...
The following map shows areas that have more tweets from locals (orange) or tourists (purple). Note the values indicate the log of the absolute difference between number of tweets from each user group. If a hex is purple and has a value of 2.302585, this means there are 10 more tweets from tourists than locals at that location (log of 10 = 2.302585).
...
- could remove "santa barbara", "california", see if we can get a better sense of what people actually care about.
...
This graph shows average daily scores over time. Above 0 is positive, below 0 is negative. We see that tweets are mostly positive and growing over time.
