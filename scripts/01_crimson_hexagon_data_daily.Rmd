...
mapview::mapview(hex_tweet_count %>% filter(tweet_count > 0))
```
...
ggmap(sb.zoom.map,  legend="none") +
  coord_equal() +
    labs(x = NULL, y = NULL) +
    theme(axis.text = element_blank()) +
    geom_hex(data = tweet_df_w_user_type, bins = 50)
```
...
```{r}
delavina_tweets <- tweet_df %>%
  filter(geo_coordinates == "c(34.4258, -119.714)")

kable(sample_n(delavina_tweets, 20)) %>%
...
mapview::mapview(hex_tweet_count %>% filter(tweet_count > 0), #remove hexes with no tweets
